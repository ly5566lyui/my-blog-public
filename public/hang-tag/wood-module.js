...
function mountWoodBadge(canvas, opts) {
	...
	function buildEnvironment() {
		...
		env.add(new THREE.Mesh(new THREE.SphereGeometry(28, 18, 18), new THREE.MeshBasicMaterial({ color: 0x8f9277, side: THREE.BackSide })))
	...
	const shadowSurface = new THREE.Mesh(
		...
		new THREE.ShadowMaterial({ color: 0x142015, opacity: widget ? 0.1 : 0.17 })
	)
	shadowSurface.position.set(0, widget ? -0.15 : 0, widget ? -0.5 : -2.08)
	...
	const halo = new THREE.Mesh(
		...
		new THREE.ShaderMaterial({
			transparent: true,
			depthWrite: false,
			uniforms: { uStrength: { value: widget ? 0.032 : 0 } },
			vertexShader: 'varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}',
			fragmentShader:
				'varying vec2 vUv;uniform float uStrength;void main(){vec2 p=(vUv-.5)*2.;float a=pow(smoothstep(1.,0.,length(p)),2.8)*uStrength;gl_FragColor=vec4(.36,.52,.20,a);}'
		})
	)
	halo.position.set(widget ? 0.15 : 0.5, widget ? -0.2 : -0.3, -2.2)
	halo.visible = true
	scene.add(halo)
	...
	for (let i = 0; i < dustCount; i++) {
		...
		dustPositions[i * 3 + 1] = (Math.random() - 0.5) * 13
		dustPositions[i * 3 + 2] = -4 + Math.random() * 6
	}
	dustGeometry.setAttribute('position', new THREE.BufferAttribute(dustPositions, 3))
	const dust = new THREE.Points(dustGeometry, new THREE.PointsMaterial({ color: 0xd8cfad, size: 0.019, transparent: true, opacity: 0.27, depthWrite: false }))
	dust.visible = !widget
	scene.add(dust)
	...
	warmLight.position.set(4, -0.5, 5)
	scene.add(warmLight)
	...
	function drawLeafMark(ctx, x, y, scale, color) {
		...
		ctx.beginPath()
		ctx.moveTo(0, 72)
		ctx.bezierCurveTo(-3, 40, 8, 12, 38, -30)
		ctx.stroke()
		;[
			[4, 38, -27, 17],
			[10, 16, -30, -3],
			...
			ctx.beginPath()
			ctx.moveTo(sx, sy)
			ctx.quadraticCurveTo((sx + ex) * 0.4, (sy + ey) * 0.45, ex, ey)
			ctx.stroke()
		})
		ctx.restore()
	}
	...
	function makeWoodTexture(side) {
		...
		gradient.addColorStop(0, '#b98a58')
		gradient.addColorStop(0.48, '#98673f')
		gradient.addColorStop(1, '#74492d')
		...
		for (let i = 0; i < 88; i++) {
			const y = rnd() * H,
				amp = 4 + rnd() * 17,
				freq = 0.008 + rnd() * 0.018
			ctx.beginPath()
			ctx.moveTo(-20, y)
			for (let x = -20; x <= W + 20; x += 12) ctx.lineTo(x, y + Math.sin(x * freq + rnd() * 1.8) * amp + Math.sin(x * 0.004) * 7)
			ctx.strokeStyle = i % 3 === 0 ? '#3a1f0d' : '#c89960'
		...
		for (let i = 0; i < 5; i++) {
			...
			for (let r = 10; r < 58; r += 8) {
				ctx.beginPath()
				ctx.ellipse(x, y, r * 1.7, r * 0.55, -0.18, 0, Math.PI * 2)
				ctx.strokeStyle = '#351b0b'
				ctx.lineWidth = 2
				ctx.stroke()
			}
		}
		...
		const ink = '#21150d',
			pale = '#f4e6c4',
			green = '#2f4931'
		ctx.strokeStyle = 'rgba(42,23,10,.42)'
		ctx.lineWidth = 3
		roundRect(ctx, 34, 34, W - 68, H - 68, 34)
		ctx.stroke()
		ctx.strokeStyle = 'rgba(229,211,171,.18)'
		ctx.lineWidth = 1
		...
		if (side === 'front') {
			...
			ctx.fillText('SUI XIN JI', 68, 92)
			ctx.fillStyle = ink
			ctx.font = '500 17px monospace'
			ctx.fillText('FIELD NOTES / BLOG', 68, 128)
			ctx.strokeStyle = 'rgba(37,23,13,.55)'
			ctx.beginPath()
			ctx.moveTo(68, 157)
			ctx.lineTo(832, 157)
			ctx.stroke()
			...
			ctx.fillText('Write as you please.', 68, 470)
			ctx.fillStyle = ink
			ctx.font = '700 92px "Noto Serif SC", "Songti SC", "STSong", serif'
			ctx.fillText('随心记', 62, 660)
			ctx.font = '500 25px monospace'
			ctx.fillText('随笔 · 灵感 · 日常', 68, 730)
			ctx.strokeStyle = 'rgba(37,23,13,.52)'
			ctx.beginPath()
			ctx.moveTo(68, 790)
			ctx.lineTo(832, 790)
			ctx.stroke()
			...
			ctx.fillText('RECORD', 68, 852)
			ctx.fillText('MOOD', 360, 852)
			ctx.fillText('SEASON', 650, 852)
			ctx.fillStyle = ink
			ctx.font = '500 23px monospace'
			ctx.fillText('VOL—01', 68, 894)
			ctx.fillText('CALM', 360, 894)
			ctx.fillText('AUTUMN', 650, 894)
			ctx.fillStyle = green
			ctx.fillRect(68, 1080, 330, 3)
			ctx.font = '600 17px monospace'
			ctx.fillText('KEEP CLOSE TO THE WILD', 68, 1126)
		} else {
			ctx.fillStyle = pale
			ctx.font = '600 21px Georgia, serif'
			ctx.fillText('FIELD NOTES', 68, 94)
			ctx.fillStyle = ink
			ctx.font = 'italic 34px Georgia, serif'
			ctx.fillText('Casual archive', 68, 158)
			ctx.strokeStyle = 'rgba(37,23,13,.55)'
			ctx.beginPath()
			ctx.moveTo(68, 190)
			ctx.lineTo(832, 190)
			...
			ctx.fillText('随心 / FREE MIND', 68, 304)
			ctx.font = '500 19px monospace'
			ctx.fillText('TYPE     NOTES', 68, 362)
			ctx.fillText('SCOPE    EVERYDAY', 68, 406)
			ctx.fillText('STATUS   ALIVE', 68, 450)
			ctx.fillStyle = 'rgba(229,211,171,.78)'
			roundRect(ctx, 62, 535, 776, 248, 18)
			ctx.fill()
			ctx.fillStyle = ink
			ctx.font = 'italic 26px Georgia, serif'
			ctx.fillText('"Let the line follow the light,', 94, 607)
			ctx.fillText('and let the leaf remember rain."', 94, 648)
			ctx.font = '500 17px monospace'
			ctx.fillText('PENNED BY THE BLOG OWNER', 94, 724)
			ctx.fillStyle = green
			ctx.fillRect(68, 1026, 390, 3)
			ctx.font = '600 17px monospace'
			ctx.fillText('CLICK THE PLAQUE TO READ', 68, 1070)
		}
	...
	const shell = new THREE.Mesh(new RoundedBoxGeometry(CFG.plaqueW, CFG.plaqueH, CFG.plaqueD, 7, 0.18), woodMaterial)
	shell.castShadow = true
	shell.receiveShadow = true
	plaqueRig.add(shell)

	const faceGeometry = new THREE.PlaneGeometry(CFG.plaqueW - 0.15, CFG.plaqueH - 0.15)
	const frontFace = new THREE.Mesh(
		faceGeometry,
		new THREE.MeshStandardMaterial({ map: makeWoodTexture('front'), roughness: 0.68, metalness: 0, envMapIntensity: 0.65 })
	)
	frontFace.position.z = CFG.plaqueD / 2 + 0.004
	const backFace = new THREE.Mesh(
		faceGeometry,
		new THREE.MeshStandardMaterial({ map: makeWoodTexture('back'), roughness: 0.68, metalness: 0, envMapIntensity: 0.65 })
	)
	backFace.position.z = -CFG.plaqueD / 2 - 0.004
	backFace.rotation.y = Math.PI
	plaqueRig.add(frontFace, backFace)

	const brass = new THREE.MeshPhysicalMaterial({ color: 0x9a7949, roughness: 0.36, metalness: 0.72, clearcoat: 0.2, envMapIntensity: 1 })
	const darkHole = new THREE.MeshStandardMaterial({ color: 0x160d07, roughness: 0.9 })
	const hole = new THREE.Mesh(new THREE.CylinderGeometry(0.145, 0.145, CFG.plaqueD + 0.05, 28), darkHole)
	hole.rotation.x = Math.PI / 2
	hole.position.set(0, CFG.eyeY, 0)
	plaqueRig.add(hole)
	const eyeletFront = new THREE.Mesh(new THREE.TorusGeometry(0.19, 0.047, 12, 36), brass)
	eyeletFront.position.set(0, CFG.eyeY, CFG.plaqueD / 2 + 0.055)
	const eyeletBack = eyeletFront.clone()
	eyeletBack.position.z = -CFG.plaqueD / 2 - 0.055
	plaqueRig.add(eyeletFront, eyeletBack)

	/* Leaf and vine builders --------------------------------------------- */
	const leafShape = new THREE.Shape()
	leafShape.moveTo(0, 0)
	leafShape.bezierCurveTo(0.16, 0.06, 0.24, 0.22, 0, 0.43)
	leafShape.bezierCurveTo(-0.24, 0.22, -0.16, 0.06, 0, 0)
	const leafGeometry = new THREE.ShapeGeometry(leafShape, 8)
	leafGeometry.computeVertexNormals()
	const leafMaterials = [
		new THREE.MeshStandardMaterial({ color: 0x4f7040, roughness: 0.78, side: THREE.DoubleSide }),
		new THREE.MeshStandardMaterial({ color: 0x78915a, roughness: 0.76, side: THREE.DoubleSide }),
		new THREE.MeshStandardMaterial({ color: 0x385438, roughness: 0.8, side: THREE.DoubleSide })
	]
	const vineMaterial = new THREE.MeshStandardMaterial({ color: 0x3f6138, roughness: 0.84, metalness: 0 })
	const youngVineMaterial = new THREE.MeshStandardMaterial({ color: 0x718957, roughness: 0.8, metalness: 0 })
	...
	function addStaticVine(points, radius, material, phase) {
		...
		for (let i = 1; i <= 7; i++) {
			...
			leaf.rotation.z = (i % 2 ? 1 : -1) * (0.72 + (i % 3) * 0.13)
			leaf.rotation.y = Math.sin(i * 1.7 + phase) * 0.45
			leaf.scale.setScalar(0.72 + (i % 3) * 0.12)
			leaf.castShadow = true
			holder.add(leaf)
			plaqueRig.add(holder)
			leaves.push({ holder, leaf, phase: i * 1.9 + phase })
		}
		return leaves
	}
	...
	plaqueLeaves.push(
		...addStaticVine(
			[
				new THREE.Vector3(-1.12, -1.75, 0.19),
				new THREE.Vector3(-1.34, -1.2, 0.18),
				new THREE.Vector3(-1.18, -0.65, -0.19),
				new THREE.Vector3(-1.34, -0.08, 0.2),
				new THREE.Vector3(-1.19, 0.55, 0.2),
				new THREE.Vector3(-1.31, 1.12, -0.18),
				new THREE.Vector3(-0.86, 1.74, 0.2),
				new THREE.Vector3(-0.28, 1.83, 0.2)
			],
			0.035,
			vineMaterial,
			0
	...
	plaqueLeaves.push(
		...addStaticVine(
			[
				new THREE.Vector3(1.14, -1.67, -0.18),
				new THREE.Vector3(1.31, -1.12, 0.19),
				new THREE.Vector3(1.18, -0.48, 0.2),
				new THREE.Vector3(1.31, 0.12, -0.18),
				new THREE.Vector3(1.17, 0.72, 0.2),
				new THREE.Vector3(0.95, 1.15, 0.2)
			],
			0.03,
			youngVineMaterial,
			1
	...
	anchorGroup.position.copy(CFG.anchor)
	const branchMaterial = new THREE.MeshStandardMaterial({ color: 0x5a371d, roughness: 0.86 })
	const anchorBar = new THREE.Mesh(new THREE.CylinderGeometry(0.075, 0.095, 1.12, 14), branchMaterial)
	anchorBar.rotation.z = Math.PI / 2
	anchorBar.rotation.y = 0.15
	anchorBar.castShadow = true
	const knot = new THREE.Mesh(new THREE.TorusGeometry(0.13, 0.035, 9, 28), vineMaterial)
	knot.rotation.x = Math.PI / 2
	knot.position.y = -0.04
	anchorGroup.add(anchorBar, knot)
	scene.add(anchorGroup) /* Elastic rope --------------------------------------------------------- */
	class Cord {
		constructor() {
		...
		reset(entrance) {
			...
			for (let i = 0; i < this.count; i++) {
				...
				const x = entrance ? Math.sin(t * Math.PI) * 0.22 + t * 0.42 : 0
				this.points[i].set(CFG.anchor.x + x, CFG.anchor.y - CFG.ropeLength * (1 + this.stretch) * t, Math.sin(t * Math.PI) * 0.05)
				this.previous[i].copy(this.points[i])
			}
			if (entrance && !prefersReducedMotion) this.previous[this.count - 1].x += 0.12
		}
		updateElasticity(dt, dragTarget) {
			...
			this.stretchVelocity += force * dt
			this.stretch += this.stretchVelocity * dt
			if (this.stretch < -0.035) {
				this.stretch = -0.035
				if (this.stretchVelocity < 0) this.stretchVelocity *= -0.22
			}
			if (this.stretch > CFG.ropeSafetyStretch) {
				this.stretch = CFG.ropeSafetyStretch
				if (this.stretchVelocity > 0) this.stretchVelocity *= -0.18
			}
			this.segment = this.baseSegment * (1 + this.stretch)
		...
		integrate(dt, dragTarget) {
			...
			if (dragTarget) this.tip.lerp(dragTarget, 0.42)
		}
		constrain() {
			...
			for (let pass = 0; pass < CFG.iterations; pass++) {
				for (let i = 0; i < this.count - 1; i++) {
					...
					if (i === 0) {
						...
						const weight = i === this.count - 2 ? 0.35 : 0.5
						a.x += dx * correction * (1 - weight)
						a.y += dy * correction * (1 - weight)
						a.z += dz * correction * (1 - weight)
		...
		step(dt, target) {
			this.updateElasticity(dt, target)
			this.integrate(dt, target)
			this.constrain()
		}
		pluck(strength) {
			this.stretchVelocity += strength
		}
		kick(x, y, z) {
			for (let i = 1; i < this.count; i++) {
				const t = i / (this.count - 1)
				this.previous[i].x -= x * t * CFG.fixedStep
				this.previous[i].y -= y * t * CFG.fixedStep
				this.previous[i].z -= z * t * CFG.fixedStep
			}
		}
		get tip() {
	...
	function makeHempTexture() {
		...
		c.height = 128
		const ctx = c.getContext('2d')
		ctx.fillStyle = '#8c724d'
		ctx.fillRect(0, 0, 64, 128)
		ctx.strokeStyle = '#c2a774'
		ctx.lineWidth = 10
		for (let x = -128; x < 128; x += 20) {
			ctx.beginPath()
			ctx.moveTo(x, 128)
			ctx.lineTo(x + 128, 0)
			ctx.stroke()
		}
		ctx.strokeStyle = '#554127'
		ctx.lineWidth = 3
		for (let x = -128; x < 128; x += 20) {
			ctx.beginPath()
			ctx.moveTo(x + 8, 128)
		...
		texture.repeat.set(1.2, 12)
		return texture
	}

	function createTube(count, radial, material) {
		const positions = new Float32Array(count * radial * 3),
			normals = new Float32Array(count * radial * 3),
			uvs = new Float32Array(count * radial * 2),
			indices = []
	...
	const ropeMaterial = new THREE.MeshStandardMaterial({ map: makeHempTexture(), color: 0xc1a373, roughness: 0.88, metalness: 0, envMapIntensity: 0.48 })
	...
	for (let i = 6; i < cord.count - 3; i += 5) {
		...
		leaf.rotation.z = (i % 2 ? 1 : -1) * 0.9
		leaf.scale.setScalar(0.62 + (i % 3) * 0.08)
		leaf.castShadow = true
		holder.add(leaf)
		scene.add(holder)
		ropeLeaves.push({ index: i, holder, leaf, phase: i * 0.71 })
	}

	const workTangent = new THREE.Vector3(),
		workNormal = new THREE.Vector3(),
		workBinormal = new THREE.Vector3(),
		workOffset = new THREE.Vector3(),
		workTemp = new THREE.Vector3()
	function updateTube(geometry, radial, centers, radius, initialNormal, captureFrames) {
		...
		for (let i = 0; i < centers.length; i++) {
			const prev = centers[Math.max(0, i - 1)],
				next = centers[Math.min(centers.length - 1, i + 1)]
			...
			workBinormal.crossVectors(workTangent, workNormal).normalize()
			if (captureFrames) {
				ropeNormals[i].copy(workNormal)
				ropeBinormals[i].copy(workBinormal)
			}
			...
			for (let j = 0; j < radial; j++) {
				...
				workOffset
					...
					.addScaledVector(workBinormal, Math.sin(angle) * r)
				const k = i * radial + j
	...
	function updateRopeGeometry(elapsed) {
		updateTube(ropeTube.geometry, ropeTube.radial, cord.points, i => clamp(0.052 * (1 - cord.stretch * 1.35), 0.036, 0.053), new THREE.Vector3(1, 0, 0), true)
		for (let i = 0; i < cord.count; i++) {
			const phase = i * 0.84
			vineCenters[i]
				.copy(cord.points[i])
				.addScaledVector(ropeNormals[i], Math.cos(phase) * 0.077)
				.addScaledVector(ropeBinormals[i], Math.sin(phase) * 0.077)
		}
		updateTube(ropeVineTube.geometry, ropeVineTube.radial, vineCenters, 0.021, new THREE.Vector3(1, 0, 0), false)
		ropeLeaves.forEach(entry => {
			const i = entry.index,
				a = vineCenters[Math.max(0, i - 1)],
				b = vineCenters[Math.min(cord.count - 1, i + 1)]
			...
			entry.leaf.rotation.y = Math.sin(elapsed * 1.35 + entry.phase) * 0.16
		})
	} /* Interaction ---------------------------------------------------------- */
	...
	const motion = {
		...
		hoverTarget: 0,
		dragging: false,
		dragCenter: new THREE.Vector3(),
		dragPrevious: new THREE.Vector3(),
	...
	const dragHit = new THREE.Vector3(),
		...
		eyeWorldOffset = new THREE.Vector3()
	const euler = new THREE.Euler(0, 0, 0, 'XYZ'),
		plaqueQuaternion = new THREE.Quaternion(),
		lastTip = cord.tip.clone()
	...
	function beginDrag(event) {
		...
		if (raycaster.ray.intersectPlane(dragPlane, dragHit)) dragOffset.subVectors(plaqueRig.position, dragHit)
	...
	function moveDrag(event) {
		...
		if (widget) {
			...
			motion.dragCenter.y = clamp(motion.dragCenter.y, -5.9, CFG.anchor.y - 0.3)
		}
	...
	function endDrag(event) {
		...
		motion.velocityZ += clamp(-motion.dragVelocity.x * 0.036, -0.46, 0.46)
	...
	function updatePlaqueMotion(dt, elapsed) {
		const tipVelocity = workTemp.subVectors(cord.tip, lastTip).multiplyScalar(1 / Math.max(dt, 0.0001))
		lastTip.copy(cord.tip)
		const beforeTip = cord.points[cord.count - 2]
		const segmentX = cord.tip.x - beforeTip.x,
			segmentZ = cord.tip.z - beforeTip.z
		const targetZ = clamp(-tipVelocity.x * 0.035 - segmentX * 1.8, -0.62, 0.62)
		const targetX = clamp(tipVelocity.z * 0.025 + segmentZ * 1.2, -0.38, 0.38)
		...
		if (motion.flipping) {
			...
			if (phase < 0.2) {
				const lead = phase / 0.2
				const eased = lead * lead * (3 - 2 * lead)
				motion.flip = motion.flipFrom + motion.flipDirection * 0.1 * eased
				stagedTiltX = -0.14 * eased
				stagedTiltZ = motion.flipDirection * 0.035 * eased
				stagedScale = 0.018 * eased
			} else if (phase < 0.82) {
				const turn = (phase - 0.2) / 0.62
				const eased = 0.5 - Math.cos(turn * Math.PI) * 0.5
				const arc = Math.sin(turn * Math.PI)
				motion.flip = motion.flipFrom + motion.flipDirection * (0.1 + (Math.PI - 0.1) * eased)
				stagedTiltX = -0.14 * (1 - eased) - 0.045 * arc
				stagedTiltZ = motion.flipDirection * (0.035 * (1 - eased) + 0.055 * arc)
				stagedScale = 0.018 + 0.018 * arc
			} else {
				const settle = (phase - 0.82) / 0.18
				const eased = 1 - Math.pow(1 - settle, 3)
				motion.flip = motion.flipTarget + motion.flipDirection * Math.sin(settle * Math.PI) * 0.045
				stagedTiltX = -0.045 * (1 - eased)
				stagedTiltZ = motion.flipDirection * 0.025 * (1 - eased)
				stagedScale = 0.018 * (1 - eased)
			}
			if (phase >= 1) {
		...
		plaqueRig.scale.setScalar(1 + motion.hover * 0.018 + stagedScale)
		woodMaterial.clearcoat = 0.16 + motion.hover * 0.12
		plaqueLeaves.forEach(entry => {
			entry.leaf.rotation.x = Math.sin(elapsed * 1.2 + entry.phase) * 0.08
		})
	}
	...
	function confinePlaque() {
		const px = plaqueRig.position.x,
			py = plaqueRig.position.y
		...
		motion.velocityZ *= 0.55
		motion.velocityX *= 0.55
	}
	...
	function resize() {
		...
		if (widget) {
			...
			const objectTop = CFG.anchor.y + 0.2
			const objectBottom = plaqueCenterY - CFG.plaqueH / 2 - 0.38
			const baseHalfH = (objectTop - objectBottom) * 0.5
			const halfH = Math.max(baseHalfH, (CFG.plaqueW / 2 + 0.3) / camera.aspect)
			const camY = (objectTop + objectBottom) * 0.5
			const dist = halfH / tan
			cameraBase.set(0, camY, dist)
			camera.position.copy(cameraBase)
			lookTarget.set(0, camY, 0)
			const marginX = CFG.plaqueW / 2 + 0.24,
				marginY = CFG.plaqueH / 2 + 0.24
			viewLimits.maxX = Math.max(0.25, halfH * camera.aspect - marginX)
			viewLimits.minY = camY - halfH + marginY
			viewLimits.maxY = Math.min(CFG.anchor.y - 0.3, camY + halfH - marginY)
		} else {
			camera.fov = width / height < 0.72 ? 40 : width / height < 1 ? 38 : 35
			const dist = width / height < 0.72 ? 14.2 : width / height < 1 ? 14.2 : 13.7
			cameraBase.set(0, 0.34, dist)
	...
	function frame() {
		...
		accumulator += dt
		let steps = 0
		while (accumulator >= CFG.fixedStep && steps < 8) {
			cord.step(CFG.fixedStep, targetEyePosition())
		...
		halo.material.uniforms.uStrength.value = (widget ? 0.034 : 0.13) + tensionRatio * 0.025
		if (backdrop) backdrop.material.uniforms.uTime.value = elapsed
		if (!prefersReducedMotion && !motion.dragging) {
			camera.position.x += (Math.sin(elapsed * 0.16) * 0.04 - camera.position.x) * 0.012
			camera.position.y += (cameraBase.y + Math.sin(elapsed * 0.2) * 0.025 - camera.position.y) * 0.012
			camera.lookAt(lookTarget)
			dust.rotation.y = elapsed * 0.006
			warmLight.intensity = (widget ? 8.8 : 17.5) + Math.sin(elapsed * 0.65) * (widget ? 0.35 : 1.2)
		}
		reportPlaqueBox()
	...
	function reportPlaqueBox() {
		...
		const hw = CFG.plaqueW / 2 + 0.34,
			hh = CFG.plaqueH / 2 + 0.34,
			hd = CFG.plaqueD / 2 + 0.2
		const rect = canvas.getBoundingClientRect()
		let minX = Infinity,
			minY = Infinity,
			maxX = -Infinity,
		...
		if (Math.abs(box.x - lastBox.x) < 1 && Math.abs(box.y - lastBox.y) < 1 && Math.abs(box.w - lastBox.w) < 1 && Math.abs(box.h - lastBox.h) < 1) return
		lastBox.x = box.x
		lastBox.y = box.y
		lastBox.w = box.w
		lastBox.h = box.h
	...
	return {
		pulse() {
			...
			cord.pluck(prefersReducedMotion ? 0.2 : 1.08)
			cord.kick(direction * 1.2, -1.8, 0.75)
			motion.velocityZ -= direction * 0.1
			motion.velocityX += 0.06
		},
		flip() {
		...
		reset() {
			...
			motion.flipping = false
			motion.dragging = false
			motion.hover = motion.hoverTarget = 0
		},
		dispose() {
			renderer.setAnimationLoop(null)
