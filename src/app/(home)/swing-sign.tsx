...
export default function SwingSign() {
		...
		;(async () => {
			try {
				...
				const api = mount(canvas, {
					widget: true,
					onPick: () => router.push(TARGET_HREF),
				})
	...
	return (
		<div className='pointer-events-none fixed left-1/2 top-0 z-30 -translate-x-1/2' aria-label='随心记'>
			<div className='relative h-[196px] w-[136px] max-sm:h-[164px] max-sm:w-[114px]'>
				<canvas
					ref={canvasRef}
					className='pointer-events-auto block h-full w-full touch-none'
					style={{ opacity: ready ? 1 : 0, transition: 'opacity .7s cubic-bezier(.22,1,.36,1)' }}
				/>
			</div>
		</div>
	)
