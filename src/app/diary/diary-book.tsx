...
import { ArrowLeft, BookOpen, CalendarDays, Edit3, KeyRound, Leaf, LoaderCircle, Plus, Save, Trash2, X } from 'lucide-react'
import { toast } from 'sonner'
import { useAuthStore } from '@/hooks/use-auth'
import { verifyAuth } from '@/lib/auth'
import { readFileAsText } from '@/lib/file-utils'
import initialEntries from './list.json'
import { pushDiaryEntries } from './services/push-diary-entries'
...
export default function DiaryBook() {
	...
	const [originalEntries, setOriginalEntries] = useState<DiaryEntry[]>(initialEntries as DiaryEntry[])
	...
	const enterManageMode = () => {
		requireAuth(() => setManageMode(true))
	}
	...
	const publishEntries = async () => {
		setIsSaving(true)
		try {
			await pushDiaryEntries(entries)
	...
	const handlePrivateKeySelection = async (file: File) => {
		...
		try {
			...
			if (!verified) {
				...
				toast.error('密钥无效或暂时无法连接 GitHub')
				return
			}
			...
			pendingActionRef.current = null
			setAuthStatus('idle')
			toast.error('密钥文件无法读取，请选择正确的 PEM 文件')
		}
	}
				...
				<div className='mx-auto w-full max-w-[920px]'>
					<header className='mb-8 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between'>
						<div>
							...
							<p className='mt-3 max-w-xl text-sm leading-7 text-[#66716a] sm:text-base'>藤蔓缠住四季，也收好每一页公开的日常。</p>
						</div>
							...
							{manageMode ? (
								<>
									<button
										...
										disabled={isSaving}
										className='inline-flex min-h-12 items-center justify-center rounded-full border border-[#cbd7c7] bg-white/55 px-5 text-sm text-[#516254] transition hover:bg-white/80 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#55784e] disabled:opacity-60'>
										取消
									</button>
									<button
										type='button'
										...
										disabled={isSaving}
										className='inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-[#b9cbb5] bg-white/65 px-5 text-sm font-medium text-[#3f6248] transition hover:-translate-y-0.5 hover:bg-white/85 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#55784e] disabled:opacity-60'>
										<Plus aria-hidden='true' className='size-4' />
										写一篇
									</button>
									<button
										type='button'
										onClick={() => requireAuth(() => void publishEntries())}
										disabled={isSaving}
										className='inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#3f6248] px-6 text-sm font-medium text-white shadow-[0_14px_30px_-18px_rgba(39,70,47,0.9)] transition hover:-translate-y-0.5 hover:bg-[#31543c] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#55784e] disabled:cursor-wait disabled:opacity-60 disabled:hover:translate-y-0'>
								...
								<button
									...
									disabled={authStatus === 'checking' || authStatus === 'validating'}
									className='inline-flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-full border border-[#cbd7c7] bg-white/55 px-5 text-sm font-medium text-[#4f6353] shadow-sm backdrop-blur-md transition hover:-translate-y-0.5 hover:bg-white/80 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#55784e] disabled:cursor-wait disabled:opacity-60 disabled:hover:translate-y-0'>
									{authStatus === 'checking' || authStatus === 'validating' ? (
										<LoaderCircle aria-hidden='true' className='size-4 animate-spin' />
									) : (
										<KeyRound aria-hidden='true' className='size-4' />
									)}
									{authStatus === 'checking' ? '检查权限' : authStatus === 'validating' ? '验证密钥' : '管理日记'}
								</button>
							)}
						</div>
					</header>

					{manageMode && editorOpen && (
					...
					{!hydrated ? (
						...
						<section className='rounded-[32px] border border-dashed border-[#aebda9] bg-white/35 px-6 py-16 text-center backdrop-blur-md'>
							...
							<p className='mt-2 text-sm leading-6 text-[#6d786f]'>{manageMode ? '从一句话开始，留下今天的第一页。' : '藤蔓正在等待第一段故事。'}</p>
							{manageMode && (
								<button
									type='button'
									onClick={openNewEntry}
